...
dim(non_outlier)
# He couldn't remove the outliers and his method of removing them was incorrect
boxplot(non_outlier$NewVolSales)$out
...
mmix$price_bkt
mmix$price_bkt <- with(mmix, cut(x = Base.Price, breaks = quantile(Base.Price), labels = c("Low", "Average", "High", "Very High"), include.lowest = TRUE,ordered_result = TRUE))
mmix %>% group_by(price_bkt) %>% summarise(min(Base.Price), max(Base.Price), count = n())
...
Reg<-lm(NewVolSales~Price_Bkt,data=mmix)
summary(Reg)
...
library(car)
vif(Mulreg)
qplot(Mulreg$residuals)
qqPlot(Mulreg)
qplot(Mulreg$fitted.values, Mulreg$residuals)
#--------------------------------Testing models -------------------------------------
